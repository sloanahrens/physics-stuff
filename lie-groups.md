...
Lie groups are continuous symmetry groups that appear throughout physics—rotations, Lorentz transformations, gauge symmetries. Their infinitesimal structure, encoded in Lie algebras, connects to [Hamiltonian mechanics](hamiltonian-mechanics.md) via Poisson brackets and to [quantum mechanics](quantum-mechanics.md) via commutators. This document requires [linear algebra](linear-algebra.md) (matrices, exponentials, eigenvalues) and [calculus](calculus-primer.md) (Taylor series). The Lorentz group underlies [special relativity](special-relativity.md); SU(2) describes [spin angular momentum](spin-angular-momentum.md); spinor representations appear in the [Dirac equation](dirac-equation.md).
