...
The path integral formulation, developed by Richard Feynman, provides an alternative to the Schrödinger equation that makes the connection to classical mechanics transparent. It sums over all possible paths, weighted by the exponential of the action—directly linking [quantum mechanics](quantum-mechanics.md) to [Lagrangian mechanics](lagrangian-mechanics.md). This approach requires [calculus](calculus-primer.md) (especially integration) and [probability](probability-statistics.md) concepts.
...
In [statistical mechanics](statistical-mechanics.md), the partition function is:
